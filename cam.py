...
# Initialize Session State (데이터 유실 방지 잠금장치)
...
if "cam_step" not in st.session_state: st.session_state.cam_step = 1  # 1: 제품촬영, 2: 라벨촬영, 3: 완료
...
with col1:
    ...
    def generate_pdf_bytes():
        ...
        if st.session_state.prod_img_data:
            ...
            c.setFont(final_font_bold, 10)
            c.setFillColor(colors.HexColor("#2C3E50"))
            c.drawString(45, y_photo_text, "[ Image 01: Product Frame ]")
        ...
        if st.session_state.label_img_data:
            ...
            c.setFont(final_font_bold, 10)
            c.setFillColor(colors.HexColor("#2C3E50"))
            c.drawString(325, y_photo_text, "[ Image 02: Label Frame ]")
    ...
    # 다운로드 버튼 (두 사진이 모두 존재할 때만 활성화시켜 누락 방지)
    if st.session_state.prod_img_data and st.session_state.label_img_data:
        ...
        st.warning("⚠️ Please complete Step 1 and Step 2 captures to enable PDF download.")

# --- RIGHT COLUMN: Sequenced Camera Board ---
with col2:
    # [수정 핵심] 단계별 카메라 제어 보드 (충돌 방지 회로)
    if st.session_state.cam_step == 1:
        ...
        if cam_prod is not None:
            st.session_state.prod_img_data = Image.open(cam_prod)
            st.success("✅ Product Photo Captured!")
            if st.button("Proceed to Step 2 (Label Capture) ➡️"):
        ...
        if cam_label is not None:
            ...
            st.success("✅ Label Photo Captured!")
            if st.button("Finish & Lock Photos 🔒"):
                st.session_state.cam_step = 3
                st.rerun()
                
    elif st.session_state.cam_step == 3:
        st.subheader("✅ All Captures Completed")
        st.info("Both images are safely saved. You can now download the PDF on the left column.")
        if st.button("🔄 Retake / Reset Photos"):
