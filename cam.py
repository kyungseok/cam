...
with col1:
    ...
    # PDF 바이너리를 생성하는 함수 (현재 메모리 기준 실시간 스냅샷 빌드)
    def generate_pdf_bytes():
        ...
        # Photos Layout (글자 겹침 현상을 막기 위해 Y축 간격 재배치 완료)
        y_photo_title = 515
        y_photo_text = 485   # 이미지 타이틀 글자 출력 위치 (30px 하향)
        y_photo_box = 295    # 이미지 사각형 박스 시작 위치 (30px 하향)
        frame_w = 250
        frame_h = 175
    ...
    # [수정의 핵심] 별도 단계 구분을 없애고 data 아규먼트에 람다/실시간 함수를 배치하여 클릭 시점의 최신 데이터를 가져옵니다.
    st.download_button(
